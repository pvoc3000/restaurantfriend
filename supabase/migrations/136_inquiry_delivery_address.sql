-- ============================================================================
-- 136 — A DELIVERY INQUIRY NEEDS AN ADDRESS
-- ============================================================================
-- Mark, 2026-09-24: "yes, require the address for delivery" — after 135 made
-- the name, email, phone, occasion, date and time required and left the
-- address optional. A delivery with no address can be neither estimated
-- (`inquiry-delivery-quote`) nor quoted without a follow-up email.
--
-- One new state, `address_required`, returned only when the fulfillment is
-- DELIVERY and the address is blank. Checked after the fulfillment itself is
-- validated and before the date and time. The honeypot is still decided first.
--
-- 135's `create_inquiry` is reproduced WHOLE (055's rule); its markers are
-- renamed `(135)` and this migration's change is marked `<<< 136`. Same
-- signature, so the grants stand. Depends on 135. RERUNNABLE.
-- ============================================================================
...
begin
  ...
  -- <<< 136: a DELIVERY needs somewhere to deliver to (Mark, 2026-09-24:
  -- "require the address for delivery"). A pickup still needs none, and one
  -- sent anyway is still not written to `delivery_address`.
  if v_fulfillment = 'delivery' and v_address is null then
    return jsonb_build_object('ok', false, 'state', 'address_required');
  end if;
...
-- ----------------------------------------------------------------------------
-- After this, as `anon`, a delivery with every other field filled and no
-- address answers {"ok": false, "state": "address_required"}.
