-- ============================================================================
-- 134 — EXTRAS ON THE INQUIRY FORM (platters, utensils, …)
-- ============================================================================
-- Mark, 2026-09-24: "add a section at the end of the inquiry form for misc
-- items like catering platters, utensils, etc? these exist in production items
-- so they would map to the special order nicely."
--
-- 1. A FIFTH CATEGORY, `extra`: an item whose TYPE is `Misc`. The type WINS
--    over the size, which fixes a latent fault too: the two 3-gallon ice cream
--    items are Misc but sized "Regular", so switched on they would have been
--    offered as DONUTS. `inquiry_category` gains the item type as a third
--    argument; the two-argument version is dropped once its two callers (the
--    menu and the gate, reproduced whole below) stop calling it.
--
-- 2. AN EXTRA MAY BE UNPRICED. None of the Misc items has a price class and
--    tier, because staff price them by hand (measured since 2025: Catering
--    Platter $5 on 64 of 74 lines; Utensils anywhere from $0.25 each to
--    $28.95; Menu Card $0–$2.50). So the menu offers an unpriced extra with a
--    null price ("priced in your quote"), and the gate writes it at $0 with
--    the note "Price to be quoted". A donut is never offered unpriced. Give an
--    extra a price class and tier (or a per-shop override) and it prices like
--    anything else.
--
-- 3. NO MINIMUM on an extra, and it counts toward none.
--
-- 4. Catering Platter and Utensils (ea) are switched ON (Mark named them);
--    132 switched every Misc item off. Menu Card, the donut cakes and the ice
--    cream stay off until somebody switches them on.
--
-- 133's `create_inquiry` and 132's `inquiry_menu` are reproduced WHOLE with
-- every change marked `<<< 134` (055's rule). Same signatures, so the grants
-- stand. Depends on 132, 133. RERUNNABLE.
-- ============================================================================

create or replace function public.inquiry_category(p_subtype text, p_size text, p_item_type text)
returns text
language sql
immutable
as $fn$
  select case
    when lower(btrim(coalesce(p_item_type, ''))) = 'misc' then 'extra'
...
revoke all on function public.inquiry_category(text, text, text) from public, anon, authenticated;


-- ----------------------------------------------------------------------------
-- The menu (132's, reproduced)
-- ----------------------------------------------------------------------------
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    from (
      select pi.id, pi.name,
             inquiry_category(pi.subtype, pi.size, pi.item_type) as category,   -- <<< 134
       ...
       where pi.org_id = p_org_id
         ...
         and inquiry_category(pi.subtype, pi.size, pi.item_type) is not null   -- <<< 134
   ...
   -- <<< 134: an extra is offered unpriced ("priced in your quote"); a donut never is.
   where m.price is not null or m.category = 'extra';
...
-- Both callers now use the three-argument form.
drop function if exists public.inquiry_category(text, text);

update production_items
   set show_on_inquiry_form = true
 where name in ('Catering Platter', 'Utensils (ea)')
   and coalesce(item_type, '') = 'Misc';
...
-- ----------------------------------------------------------------------------
--   select count(*) from pg_proc where proname = 'inquiry_category';   -> 1
--   select i ->> 'name', i ->> 'price'
--     from jsonb_array_elements(public.inquiry_menu('<org>') -> 'items') i
--    where i ->> 'category' = 'extra';
--     -> Catering Platter | (null), Utensils (ea) | (null)
