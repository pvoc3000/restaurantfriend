// Turning a machine's reading of an bill into a record you can approve.
//
// This runs CLIENT-side, in `useAttachmentActions.read()`, and not in the
// `extract-bill` edge function — deliberately. That function is a READER:
...
export type BillCreationResult =
  /** `joined` when this reading was filed against an bill that already
   *  existed, rather than becoming a new one. */
  | { billId: string; joined: boolean }
  | { error: string };
 ...
 * themselves what "file the paperwork" does. On a multi-page bill the second
 * page JOINS the first (see `filedBillFor` and `absorbIntoInvoice`), so
...
export async function fileReadings(
  ...
  for (const reading of readings) {
    ...
    if ("error" in result) {
      return { filed: bills.size, error: result.error };
    }
    bills.add(result.billId);
  }
  return { filed: bills.size };
}

/**
 * File a reading as an bill record, and link it to the order it was read on.
 *
 ...
 *      bill, which is visible on the list and fixable by hand. Lines first
 *      would leave orphans nothing points at.
 *   2. the lines, in one insert.
 *   3. the matches, as an update per matched line.
 *   4. the attachment's `bill_id` LAST, because that is the flag which stops
 ...
 * see a second COPY of the same bill — two files, two rows, two nulls — and
 * it cannot see the same bill read on a second order. Both are ordinary: a
 * retaken photo, a second page saved separately, a consolidated bill
 * covering two POs.
 *
 ...
 *       — and since step 4 is last-writer-wins, the earlier bill is orphaned
 *       rather than found. Eight of the live duplicates had no attachment
 *       pointing at them at all, which is that race's signature.
 *   0b. an bill already on file under the same vendor and number is JOINED,
 *       not duplicated — see `filedBillFor`, which is the certain half of
 *       the warning the detail screen already shows a human.
 */
export async function createBillFromReading(
  supabase: SupabaseClient,
  {
    orgId,
    attachmentId,
  ...
  if (!vendorId || !locationId) {
    return { error: "No vendor or location to file this bill against." };
  }

  // 0a. What does the DATABASE say this document is filed under? The caller's
  // copy can be a refresh behind, and acting on it is how an bill ends up
  ...
  if (current?.bill_id) {
    return { billId: current.bill_id as string, joined: true };
  }
  ...
  const { data: onFile, error: onFileError } = await supabase
    ...
    .limit(1000);
  if (onFileError) {
    return { error: `Could not check for an existing bill: ${onFileError.message}` };
  }
  const existing = filedBillFor(
  ...
  if (existing) {
    ...
    if (joinError) {
      return { error: `Filed against bill ${existing.invoice_number ?? ""}, but ${joinError}` };
    }
    const { error: tagError } = await supabase
      .from("purchase_order_attachments")
      .update({ bill_id: existing.id })
      .eq("id", attachmentId);
    if (tagError) {
      return { error: `Found the bill already on file, but could not tag this file: ${tagError.message}` };
    }
    return { billId: existing.id, joined: true };
  }
  ...
  if (drafts.length > 0) {
    const { error: lineError } = await supabase.from("vendor_bill_lines").insert(
      drafts.map((line) => ({ ...line, org_id: orgId, bill_id: billId }))
    );
    if (lineError) {
      // The header stands. An bill with no lines is a visible, fixable state
      ...
      return { error: `Filed the bill, but its lines failed: ${lineError.message}` };
    }
  }

  // Link what the matcher can pair. The bill is filed either way: a linked
  // bill and an unlinked one are both valid records, and "Link to PO…" is
  ...
  if (order && drafts.length > 0) {
    ...
    if (linkError) return { error: `Filed the bill, but linking failed: ${linkError}` };
  }
  ...
  const { error: flagError } = await supabase
    ...
    .eq("id", attachmentId);
  if (flagError) {
    return { error: `Filed the bill, but could not tag the file: ${flagError.message}` };
  }

  return { billId, joined: false };
}

/**
 * Fold a reading into an bill that already exists — ANOTHER PAGE of a bill
...
async function absorbIntoInvoice(
  ...
  if (fresh.length > 0) {
    const { error } = await supabase
      .from("vendor_bill_lines")
      .insert(fresh.map((line) => ({ ...line, org_id: orgId, bill_id: billId })));
  ...
  // Receiving reads the lines that name ITS purchase order, so a joined bill
  // with no links would leave the second order of a consolidated bill with
  // no bill at all — worse than the duplicate this replaces.
  if (order) {
    const linkError = await linkMatchedLines(supabase, { billId, order, extraction });
...
/**
 * Write the matcher's pairing onto the bill's lines.
 *
 ...
 *
 * IT ONLY EVER FILLS AN EMPTY LINK, never replaces one. On a fresh bill
 * every line is empty and this is invisible; it earns its keep on the JOIN
 * path, where the bill already exists and some of its lines already name
 * the FIRST order they were read on. A consolidated bill covering two POs
...
async function linkMatchedLines(
  supabase: SupabaseClient,
  {
    ...
    order,
    extraction,
  }: { billId: string; order: BillCreationOrder; extraction: InvoiceExtraction }
): Promise<string | null> {
