 ...
 * `mac-primary` commit wears, so once the days became small raised buttons a
 * different grey on the ON ones read as a different kind of control. Black on
 * it is ~12.6:1.
 *
 * History, kept because it is the road to here: black (a `TabPicker`'s
 * selected cell) → `neutral-600` → `neutral-500` (#757575, 2026-09-12, the
 * lightest grey that carries WHITE type at AA, 4.61:1) → this, which the
 * 2026-09-12 note named as the next step and the one that flips the type.
 * One constant because the picker draws its cells twice, live and read-only,
 * and the two must never disagree about what "on" looks like.
 *
