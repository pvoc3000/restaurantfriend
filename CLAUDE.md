...
- **USE THE PARTS THAT EXIST — don't hand-roll a second one.** This app has no
  ...
  | `ui/Checkbox` | `<input type="checkbox">` | every checkbox — except on a `.mac-page` screen, where `ui/MacCheckbox` (a native input beside its label, the box drawn in `mac-look.css`) is the experiment |
