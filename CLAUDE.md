...
- **EVERY BUTTON IS WHITE; only a SET FILTER is black** (Mark, 2026-08-02, after
  ...
  TabPicker cell — **though not a WEEKDAY PICKER's day, which is `#757575`
  since 2026-09-12** (Mark: "dark grey instead of black", then "can we go
  lighter"). Seven butted boxes are a SET whose SHAPE you read, where a
  TabPicker's cells are one segmented bar in which the fill IS the answer, so a
  softer fill makes the on-days a pattern rather than seven assertions. It is
  the lightest grey that keeps WHITE type at AA — 4.61:1, where #808080 is 3.95
  and fails — so anything lighter has to flip the type to black and become
  #c0c0c0, the Mac look's own fill, which is a different decision rather than a
  shade. Black
