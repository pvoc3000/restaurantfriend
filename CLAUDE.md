...
- **USE THE PARTS THAT EXIST — don't hand-roll a second one.** This app has no
  ...
  | `ui/CalendarGrid` | a second month grid | the one calendar both `DateField` and `RangePicker` draw: Sunday first, six rows, ends filled and the days between washed, today bold, a roving tab stop with arrow keys that turn the month and never land on a disabled day |
