...
- **USE THE PARTS THAT EXIST — don't hand-roll a second one.** This app has no
  ...
  | `data-rf-calc` on the input (`ui/CalcKeys` is already mounted) | dropping `inputMode` so the full keyboard appears | letting a numeric field take a `lib/calc` expression ON AN IPAD — iOS renders `inputMode="decimal"` as the number pad, which has no operators, and on iPadOS `*`/`+`/`×`/`÷` are two keyboard layers deep. The attribute is the whole wiring: one strip mounted in the (app) layout offers `× ÷ + − ( )` above the keyboard whenever a marked field takes focus on a touch device, writing through the native value setter so a controlled React input needs no code. Mark ONLY fields `evaluateNumeric` reads — several others carry `inputMode="decimal"` and parse with a plain `Number()`, where an inserted `×` is a value that can't save |
