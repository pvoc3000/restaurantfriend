...
- **USE THE PARTS THAT EXIST — don't hand-roll a second one.** This app has no
  ...
  | `ui/TimePicker` + `lib/timeInput` | `<input type="time">` on a Mac-styled screen | a time of day chosen from a small Mac window (Mark, 2026-09-10) — a `ui/MacTitleBar` (the field's own name centred; the close box commits, like a click away), a 2px frame with the hard shadow, a black-ruled grid of hours, of minutes in fives, and AM/PM, and at the bottom an ENTRY FIELD that takes any time typed (`9:30p`, `2130`, `noon`; 24-hour without a suffix, never a guess at the half of the day) beside Clear and Set. A pick is a DRAFT, because a time is three choices and writing each would store 3:00 AM on the way to 3:30 PM: Set, Enter and CLICKING AWAY commit (`ui/CalcPad`'s leaving-is-the-commit rule), ESCAPE discards. **Escape is heard in the CAPTURE phase** — the panel hook closes, and so commits, on a bubbling `window` keydown, and in Safari a clicked cell never takes focus, so only a capture listener hears Escape before that. So far only the location record's hours block uses it; `ui/TimeField` stays everywhere else |
