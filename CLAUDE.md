...
- **USE THE PARTS THAT EXIST — don't hand-roll a second one.** This app has no
  ...
  | `ui/Checkbox` | `<input type="checkbox">` | every checkbox — except on a `.mac-page` screen, where `ui/MacCheckbox` (a native input beside its label, the box drawn in `mac-look.css`) is the experiment. Its `size="lg"` is a 24px box in a row a button tall (36px, edges still 1px/2px; a 36px box and then a 28px one read as larger than the field beside it, and the row must be block-level or a line box adds 5px beneath it), which the location record's Active field wears through `ActiveToggle appearance="mac-checkbox"` — the first row of the identity `dl`, above Code, where a switch beside the title used to be (Mark, 2026-09-10). Its word sits in the `dl`'s label column like every other field, so in that appearance the checkbox carries it as an sr-only accessible name and a read-only cell says Yes/No |
